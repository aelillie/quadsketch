...
Compression algorithms for approximate nearest neighbor is a well studied field and many papers have been published on the area. The need for compression arises as many nearest neighbor algorithm suffer from \textit{"the curse of dimensionality"} phenomenon; either space or query time are exponential in the dimension \textit{D}\cite{ilya15}. %To escape this curse, researchers proposed approximation algorithms for the problem.
\\
\\
...
\cite{wagner17} introduces the \qs{} algorithm, which competes with state of the art distance preserving compression algorithms. This paper investigates the performance complexity of \qs{} in terms of the parameters \textit{L} and $\Lambda$ and tries to introduce heuristics for improving or reducing their influence on the complexity. From \tm{1} they are defined as such: \\

$\Lambda$=\bo{$log(d log\Phi/\epsilon\delta)$}\\
